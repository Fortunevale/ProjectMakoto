    ...
    internal async Task VoiceStateUpdated(DiscordClient sender, VoiceStateUpdateEventArgs e)
    {
        if (e.After?.Channel?.Id == _bot.guilds[e.Guild.Id].VcCreator.Channel)
        {
            try
            {
                ...
                if (e.After.Channel.Parent is null || e.After.Channel.Parent.Children.Count >= 50)
                {
                    await member.DisconnectFromVoiceAsync();
                    return;
                }

                var newChannel = await e.Guild.CreateChannelAsync($"{member.DisplayName.SanitizeForCode()}'s Channel", ChannelType.Voice, e.After.Channel.Parent, default, null, 8);
                ...
                await newChannel.SendMessageAsync(new DiscordMessageBuilder().WithContent(e.User.Mention).WithEmbed(new DiscordEmbedBuilder().WithAuthor(e.Guild.Name, "", e.Guild.IconUrl).WithColor(EmbedColors.Info).WithTimestamp(DateTime.UtcNow)
                    .WithDescription($"This is your temporary personal channel.\n\nIf this channel becomes empty, it'll be deleted. Use the `/vc` commands to manage this channel.")));
            }
            catch (Exception)
            {
                try
