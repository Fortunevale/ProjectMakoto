...
internal class GenericGuildEvents
{
    ...
    internal async Task GuildMemberAdded(DiscordClient sender, GuildMemberAddEventArgs e)
    {
        Task.Run(async () =>
        {
            ...
            if (_bot._guilds[e.Guild.Id].Members[e.Member.Id].FirstJoinDate == DateTime.UnixEpoch)
                _bot._guilds[e.Guild.Id].Members[e.Member.Id].FirstJoinDate = e.Member.JoinedAt.UtcDateTime;

            if (_bot._guilds[e.Guild.Id].JoinSettings.ReApplyNickname)
                if (_bot._guilds[e.Guild.Id].Members[e.Member.Id].LastLeaveDate.ToUniversalTime().GetTimespanSince().TotalDays < 60)
                    e.Member.ModifyAsync(x => x.Nickname = _bot._guilds[e.Guild.Id].Members[e.Member.Id].SavedNickname).Add(_bot._watcher);

            Task task = Task.Run(async () =>
            {
                if (!_bot._guilds[e.Guild.Id].JoinSettings.ReApplyRoles)
                    return;
                ...
                if (_bot._guilds[e.Guild.Id].Members[e.Member.Id].MemberRoles.Count > 0)
                {
                    ...
                    foreach (var b in _bot._guilds[e.Guild.Id].Members[e.Member.Id].MemberRoles)
                    {
            ...
            _bot._guilds[e.Guild.Id].Members[e.Member.Id].LastLeaveDate = DateTime.UnixEpoch;
        }).Add(_bot._watcher);
    }
    ...
    internal async Task GuildMemberUpdated(DiscordClient sender, GuildMemberUpdateEventArgs e)
    {
        Task.Run(async () =>
        {
            ...
            _bot._guilds[e.Guild.Id].Members[e.Member.Id].SavedNickname = e.Member.Nickname;
        }).Add(_bot._watcher);
    }
