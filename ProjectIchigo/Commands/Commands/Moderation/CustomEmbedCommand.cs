        ...
        return Task.Run(async () =>
        {
            ...
            while (true)
            {
                ...
                if (Menu1.Result.Interaction.Data.CustomId == SetTitle.CustomId)
                {
                    ...
                    GeneratedEmbed.Url = Response.Interaction.GetModalValueByCustomId("url");
                    continue;
                }
                else if (Menu1.Result.Interaction.Data.CustomId == SetAuthor.CustomId)
                {
                    ...
                    else if (Menu2.Result.Interaction.Data.CustomId == SetUrl.CustomId)
                    {
                        ...
                        GeneratedEmbed.Author.Url = Response.Interaction.GetModalValueByCustomId("url");
                        continue;
                    }
                    else if (Menu2.Result.Interaction.Data.CustomId == SetIcon.CustomId)
                    {
                        ...
                        var asset = await (await ctx.Client.GetChannelAsync(ctx.Bot._status.LoadedConfig.OtherAssetsChannelId)).SendMessageAsync(new DiscordMessageBuilder().WithContent($"{ctx.User.Mention} `{ctx.User.UsernameWithDiscriminator} ({ctx.User.Id})`").WithFile($"{Guid.NewGuid()}.png", stream.stream));
                    ...
                    else if (Menu2.Result.Interaction.Data.CustomId == SetByUser.CustomId)
                    {
                        var modal = new DiscordInteractionModalBuilder("Select user by id", Guid.NewGuid().ToString())
                            .AddTextComponent(new DiscordTextComponent(TextComponentStyle.Small, "userid", "User Id", "", 0, 20, false, GeneratedEmbed.Author.Url));
                ...
                else if (Menu1.Result.Interaction.Data.CustomId == SetFooter.CustomId)
                {
                    ...
                    if (Menu2.Result.Interaction.Data.CustomId == SetText.CustomId)
                    {
                        var modal = new DiscordInteractionModalBuilder("Set text for footer field", Guid.NewGuid().ToString())
                            .AddTextComponent(new DiscordTextComponent(TextComponentStyle.Paragraph, "text", "Text", "", 0, 2048, false, GeneratedEmbed.Author.Name));
