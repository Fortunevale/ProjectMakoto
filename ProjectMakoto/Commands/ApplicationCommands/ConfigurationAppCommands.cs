...
public sealed class ConfigurationAppCommands : ApplicationCommandsModule
{
    ...
    public async Task Add(ContextMenuContext ctx) 
        => new Commands.ReactionRolesCommand.AddCommand().ExecuteCommand(ctx, _bot, new Dictionary<string, object> { { "message", ctx.TargetMessage }, }).Add(_bot.watcher, ctx);
    ...
    public async Task Remove(ContextMenuContext ctx) 
        => new Commands.ReactionRolesCommand.RemoveCommand().ExecuteCommand(ctx, _bot, new Dictionary<string, object> { { "message", ctx.TargetMessage }, }).Add(_bot.watcher, ctx);
